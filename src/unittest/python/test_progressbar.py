...
from mp4ansi.progressbar import FILL
from mp4ansi.progressbar import ALIAS_WIDTH
...
class TestProgressBar(unittest.TestCase):
    ...
    @patch('mp4ansi.progressbar.ProgressBar._get_fill')
    def test__init_Should_SetDefaults_When_Called(self, get_fill_patch, *patches):
        ...
        self.assertIsNone(pbar.message)
    ...
    def test__init_Should_SetDefaults_When_AttributesPassed(self, get_fill_patch, *patches):
        pbar = ProgressBar(0, total=100, regex={'key', 'value'})
        self.assertEqual(pbar.index, 0)
    ...
    @patch('mp4ansi.progressbar.colorama_init')
    @patch('mp4ansi.progressbar.ProgressBar._get_progress')
    def test__str_Should_ReturnExpected_When_Called(self, get_progress_patch, *patches):
    ...
    @patch('mp4ansi.progressbar.colorama_init')
    @patch('mp4ansi.progressbar.ProgressBar._match_count', return_value=True)
    ...
    def test__match_alias_Should_ReturnMatchAndSetExpected_When_RegexMatchGreaterThanWidth(self, *patches):
        pbar = ProgressBar(0, regex={'alias': r'^id is: (?P<value>.*)$'})
    ...
    def test__match_count_ShouldReturnMatchAndSetExpected_When_RegexMatch(self, *patches):
        pbar = ProgressBar(0, regex={'count': r'processed item'})
        pbar.total = 100
        text = 'processed item'
    ...
    def test__match_count_ShouldReturnNone_When_NoRegexMatch(self, *patches):
        pbar = ProgressBar(0, regex={'count': r'processed widget'})
        pbar.total = 100
        pbar.count = 10
        text = 'processed item'
        result = pbar._match_count(text)
    ...
    @patch('mp4ansi.progressbar.colorama_init')
    @patch('mp4ansi.progressbar.ProgressBar._get_complete')
